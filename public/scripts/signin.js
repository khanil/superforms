...
(() => {
	...
	function signIn() {
		...
		if(result instanceof Error) {
			...
			// console.log(bcrypt.hashSync(hash, login + Date.now() / 30000));
			sendRequest('PUT', '/api/signin', user.email)
				.then(response => {
					var hash = CryptoJS.SHA3(user.password, response.split('$')[0]).toString()
					hash = CryptoJS.HmacSHA3(hash, response.split('$')[1]).toString()
